...
NetStats 的系统指标、网络速度与 Clash Verge Dev 状态均在本机读取和处理，不会上传到 NetStats 的任何项目服务器。Clash Verge Dev 状态展示只读取本机配置、Mihomo 本地控制接口和 macOS 系统代理状态。公网 IP 与地理位置功能需要请求 `ipinfo.io` 以解析当前公网出口；如果你不需要该信息，可以通过系统防火墙或网络过滤工具阻止该请求。
