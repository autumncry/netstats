...
<p align="center">
  <strong>macOS 原生菜单栏系统监控工具，快速查看 CPU、内存、网络速度、公网 IP，并轻量控制 Clash Verge Dev。</strong>
</p>
...
- Clash Verge Dev 状态与控制需要本机已安装并运行 Clash Verge Dev
...
- 显示并控制 Clash Verge Dev：运行状态、系统代理开关、TUN 开关、规则/全局/直连模式、订阅、代理组、当前节点
- 可配置哪些指标显示在菜单栏和悬停提示中
...
NetStats 的系统指标、网络速度与 Clash Verge Dev 状态均在本机读取和处理，不会上传到 NetStats 的任何项目服务器。Clash Verge Dev 控制功能只会读取/修改本机 Clash Verge Dev 配置、Mihomo 本地控制接口和 macOS 系统代理设置。公网 IP 与地理位置功能需要请求 `ipinfo.io` 以解析当前公网出口；如果你不需要该信息，可以通过系统防火墙或网络过滤工具阻止该请求。
...
### Clash Verge Dev 控制会关闭原 App 吗？

不会。NetStats 只提供轻量控制：切换系统代理、TUN，以及规则/全局/直连模式。它不会退出或重启 Clash Verge Dev。
